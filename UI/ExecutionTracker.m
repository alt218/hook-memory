...
- (NSString *)exportJSONSummary {
    NSDictionary *payload = [self snapshot];
    NSError *err = nil;
...
- (NSString *)exportCSVSummary {
    NSMutableString *csv = [NSMutableString stringWithString:@"method,count,last_unix,last_duration_ms,total_duration_ms,blocked,mem_delta_total\n"];
    NSArray *rows = [self sortedByCount];
    for (MethodExecutionInfo *i in rows) {
        NSTimeInterval t = i.lastExecution ? i.lastExecution.timeIntervalSince1970 : 0;
        [csv appendFormat:@"\"%@\",%ld,%.3f,%.3f,%.3f,%d,%lld\n",
         (i.methodName ?: @""),
         (long)i.count,
         t,
         i.lastDurationMs,
