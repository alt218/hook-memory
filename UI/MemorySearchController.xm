...
@property(nonatomic,strong) NSArray<NSNumber *> *addresses;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vm_address_t addr = (vm_address_t)self.addresses[indexPath.row].unsignedLongLongValue;
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItems = @[self.diffButton, self.densityButton];
...
- (void)setupHeader {
    ...
    self.queryField.backgroundColor = UIColor.darkGrayColor;
    ...
    self.resetButton.layer.borderColor = UIColor.systemGreenColor.CGColor;
    ...
    self.continueButton.layer.borderColor = UIColor.systemBlueColor.CGColor;
    ...
    self.historyButton.layer.borderColor = UIColor.systemOrangeColor.CGColor;
...
- (void)updateTypeChipStyles {
    for (UIView *v in self.typeScrollView.subviews) {
        ...
        btn.layer.borderColor = selected ? UIColor.systemGreenColor.CGColor : UIColor.grayColor.CGColor;
        btn.backgroundColor = selected ? [[UIColor systemGreenColor] colorWithAlphaComponent:0.2] : UIColor.clearColor;
        [btn setTitleColor:(selected ? UIColor.systemGreenColor : UIColor.lightGrayColor) forState:UIControlStateNormal];
    }
}
...
- (NSString *)formattedHistoryDetail:(NSDictionary *)entry index:(NSInteger)index {
    ...
    for (NSUInteger i = 1; i < hits.count; i++) {
        ...
        [s appendFormat:@"[%lu] 0x%llx (delta:%+lld)\n", (unsigned long)(i + 1), addr, delta];
        if (i >= 80) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (SearchCompactMode()) {
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@%@", fav ? @"★ " : @"", value ?: @""];
    } else {
        cell.detailTextLabel.text = fav ? [NSString stringWithFormat:@"★ %@", value ?: @""] : value;
    }
...
- (void)showNearbyAddressesFor:(vm_address_t)address {
    ...
    for (NSInteger i = -4; i <= 4; i++) {
        vm_address_t a = address + ((vm_address_t)i * (vm_address_t)unit);
        [nearby addObject:@(a)];
    }
